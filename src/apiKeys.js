...
const PAID_API_KEY_TIERS = new Set(["pro", "agency"]);
...
function isPaidApiKeyTier(tier) {
  return PAID_API_KEY_TIERS.has(normalizeApiKeyTier(tier));
}
...
async function createApiKey(env, { label } = {}) {
  ...
  `).bind(id, keyHash, keyPrefix, normalizedLabel).first();
...
async function setApiKeyTier(env, apiKeyId, tier, { stripeCustomerId, stripeSubscriptionId } = {}) {
...
async function findApiKeyById(env, apiKeyId) {
...
async function findApiKeyByStripeSubscription(env, subscriptionId) {
  if (!subscriptionId) return null;
  const row = await env.DB.prepare(`
    SELECT id, key_prefix, tier, label, created_at, last_used_at, revoked,
           stripe_customer_id, stripe_subscription_id
    FROM api_keys
    WHERE stripe_subscription_id = ? AND revoked = 0
    LIMIT 1
