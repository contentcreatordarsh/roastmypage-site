...
test("createApiKey returns plaintext once and stores only a SHA-256 hash", async () => {
  ...
  assert.equal(created.apiKey.paid, false);
...
test("authenticateApiKeyRequest hashes presented key and returns active row", async () => {
  ...
  assert.equal(auth.apiKey.paid, true);
});
...
test("setApiKeyTier upgrades a key to a paid plan", async () => {
  const env = {
    DB: {
      prepare: () => ({
        bind: (tier, customer, sub, id) => ({
          first: async () => ({
            id,
  ...
  const updated = await setApiKeyTier(env, "key-1", "agency", {
    stripeCustomerId: "cus_1",
    stripeSubscriptionId: "sub_1"
  });
  assert.equal(updated.tier, "agency");
  assert.equal(updated.paid, true);
  assert.equal(isPaidApiKeyTier("free"), false);
  assert.equal(isPaidApiKeyTier("pro"), true);
});
