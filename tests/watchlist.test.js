...
test("processWatchlistAlerts ignores stored bot-challenge scores", async () => {
  ...
  assert.equal(result.checked, 1);
  assert.equal(result.alerted, 0);
  // The row rotates so it cannot starve the batch (#142), but the fabricated
  // challenge score is never recorded as the new last_score (#146).
  assert.equal(runs.length, 1);
  assert.ok(runs[0].includes("UPDATE watchlist SET updated_at"));
  assert.ok(!runs.some((sql) => sql.includes("last_score")));
});
