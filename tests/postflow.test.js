...
test("checkOperationRateLimit applies the opt-out operation limit", async () => {
  const env = { DB: mockDb({ request_count: CONFIG.RATE_LIMIT_OPTOUT_MAX + 1, window_start: new Date().toISOString() }) };
  const blocked = await checkOperationRateLimit(env, "ip-hash", "optout");
  assert.equal(blocked.allowed, false);
});
